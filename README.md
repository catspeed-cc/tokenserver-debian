...
Currently the api requires a trailing slash (ex. http://tokenserver.catspeed.cc/api/v1/get_tokens/) which is not a big deal, but I will try and fix this. Real API endpoint would not have a trailing slash. It's just some nginx configuration I have to work out.
